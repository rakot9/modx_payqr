payqr.grid.Items = function (config) {
	config = config || {};
	if (!config.id) {
		config.id = 'payqr-grid-items';
	}
	Ext.applyIf(config, {
...
Ext.extend(payqr.grid.Items, MODx.grid.Grid, {
	windows: {},
	...
	getMenu: function (grid, rowIndex) {
		...
		var menu = payqr.utils.getMenu(row.data['actions'], this, ids);
	...
	createItem: function (btn, e) {
		var w = MODx.load({
			xtype: 'payqr-item-window-create',
			id: Ext.id(),
			listeners: {
				success: {
					fn: function () {
		...
		MODx.Ajax.request({
			...
			listeners: {
				success: {
					fn: function (r) {
						var w = MODx.load({
							xtype: 'payqr-item-window-update',
							id: Ext.id(),
							record: r,
							listeners: {
								success: {
	...
	removeItem: function (act, btn, e) {
		...
		MODx.msg.confirm({
			title: ids.length > 1
				? _('payqr_items_remove')
				: _('payqr_item_remove'),
			text: ids.length > 1
				? _('payqr_items_remove_confirm')
				: _('payqr_item_remove_confirm'),
			url: this.config.url,
			params: {
	...
	getColumns: function (config) {
		return [{
			...
			dataIndex: 'actions',
			renderer: payqr.utils.renderActions,
			sortable: false,
			width: 100,
			id: 'actions'
		}];
	...
	getTopBar: function (config) {
		return [{
			text: '<i class="icon icon-plus"></i>&nbsp;' + _('payqr_item_create'),
...
Ext.reg('payqr-grid-items', payqr.grid.Items);
